-- Top spells per DCSS spell level 1–9 (3×3 grid, top 7 spells per level; see FAVOURITE_SPELLS_PER_LEVEL in lib/morgue-api.ts).
...
create table if not exists public.user_favourite_spells (
  ...
  rank int not null check (rank >= 1 and rank <= 7),
